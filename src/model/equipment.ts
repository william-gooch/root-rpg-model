...
export type TagName = typeof equipmentTags[number]["name"];

export type WeaponRange = "Intimate" | "Close" | "Far";
...
export const getItemValue = (item: EquipmentItem) => {
  const tagCost = item.tags
    .map(tag => equipmentTags.find(t => t.name === tag))
    .reduce((prev, curr) => {
      let neg = false;
      if(curr) {
